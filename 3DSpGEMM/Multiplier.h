...
SpDCCols<IT, NT>* multiply(SpDCCols<IT, NT> & splitA, SpDCCols<IT, NT> & splitB, CCGrid & CMG, bool isBT, bool threaded, bool print=true)
{
...
#pragma omp parallel
    ...
    if(CMG.myrank == 0 && print)
    {
        double time_other = time_total - (comm_bcast + comm_reduce + comp_summa + comp_reduce + comp_reduce_layer + comp_result);
        std::cerr<< " ----------------------------------------------------------------------------------------------\n";
        std::cerr<<" comm_bcast   comm_scatter comp_summa comp_merge  comp_scatter  comp_result     other      total\n";
        std::cerr<<" ----------------------------------------------------------------------------------------------\n";
